...
//!
//! Errors reported via these macros are logged at Error (actionable) or Warn (non-actionable).
//! Plain `log::error!` is also log-only.
...
///
/// If the error is actionable, it is logged at the Error level; otherwise it is logged at the Warn
/// level. Use this for explicit, structured error reporting instead of bare `log::error!`.
#[macro_export]
macro_rules! report_error {
    ...
    // Reports `err` when actionable and attaches the given fields to the local log line,
    // keeping per-instance specifics out of the primary message.
...
/// Used by `report_error!(.., extra: { .. })`. Runs `report` immediately.
#[doc(hidden)]
