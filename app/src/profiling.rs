...
/// Dumps a jemalloc heap profile to disk when excessive memory usage is detected.
///
/// On Linux the profile is produced in-process via the `jemalloc_pprof` crate
/// as a raw (unsymbolized) pprof. On other platforms it spawns the bundled
...
pub async fn dump_jemalloc_heap_profile(memory_breakdown: serde_json::Value) {
    ...
    match dump_heap_profile_to_disk().await {
        ...
        Err(err) => log::warn!("Failed to dump heap profile: {err:#}"),
    }
}
